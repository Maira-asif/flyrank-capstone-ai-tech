...
- Validation logic that's shared between Node (tests) and the browser must guard `module.exports` with `typeof module !== 'undefined'` — never assume Node-only.
- Every form input gets a `<label for>` and an `aria-describedby`-linked error region with `aria-live="polite"`; placeholder text is never a substitute for a label.
- Vague prompts ("build X") are not acceptable for anything shipped to the capstone — prompts must state field-level constraints, expected error behavior, and end with a verification step ("write tests, run them") before output is accepted.
